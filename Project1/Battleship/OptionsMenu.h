...
Options optionsFunction(sf::RenderWindow &window)
{
	...
	while (window.isOpen())
	{
		...
		backlinker.setPosition(sf::Vector2f((windowsize.x *0.05) - backpos.x / 2, 0));
		...
		invertgridword.setFillColor(optionsWindow.textColor);
		...
		statics.setFillColor(optionsWindow.textColor);
		statics.setCharacterSize(fontsize);
		statics.setString("Statistics");
		soundfxword.setFillColor(optionsWindow.textColor);
		themes.setFillColor(optionsWindow.textColor);
		soundword.setFillColor(optionsWindow.textColor);
		...
		if (LeftReleased)
		{
			...
			else if (Mospos.x >= spacepos1.x && Mospos.y >= spacepos1.y && Mospos.x <= spacepos2.x && Mospos.y <= spacepos2.y)
			{
				...
				optionsWindow.textColor = sf::Color::White;
			}
			else if (Mospos.x >= goldenpos1.x && Mospos.y >= goldenpos1.y && Mospos.x <= goldenpos2.x&& Mospos.y <= goldenpos2.y)
			{
				...
				optionsWindow.textColor = sf::Color::Black;
		...
		{
			outlinespace.setOutlineColor(optionsWindow.textColor);
			outlinegolden.setOutlineColor(optionsWindow.BackGroundColor);
		}
		else
		{
			outlinegolden.setOutlineColor(optionsWindow.textColor);
			outlinespace.setOutlineColor(optionsWindow.BackGroundColor);
		}
		window.draw(statics);
