...
char MainMenu(sf::RenderWindow& window, sf::TcpSocket &socket)
{
	...
	while (window.isOpen())
	{
		...
		MenueText.setString("Click This Side To Be HOSET                                                 Click This Side To Be CLIENT ");
		...
		MenueText.setPosition(sf::Vector2f(WindowSize.x / 2 - textsize.width / 2, (WindowSize.y*0.85) - textsize.height / 2));
		...
		if (MosPosition.x >= position1.x&&MosPosition.y >= position1.y&&MosPosition.x <= position2.x&&MosPosition.y <= position2.y)
		{
			...
			if (LeftReleased) {
				...
				if (MosPosition.x <= WindowSize.x / 2) {
					cout << "host";
					HostMenu(window, socket);
					return 'H';
				}
				else {
					ClientMenu(window, socket);
					return 'C';
				}
			}
		}
